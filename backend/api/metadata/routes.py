...
@metadata_api_bp.route('/details/<provider>/<item_id>', methods=['GET'])
def api_get_details(provider, item_id):
    """Get details for any item (book or manga).
    ...
    Returns:
        Response: The item details.
    """
    try:
        LOGGER.info(f"Fetching details for provider: {provider}, item_id: {item_id}")
        
        details = get_manga_details(item_id, provider)
        
        LOGGER.info(f"Details response for {provider}/{item_id}: {details}")
        
        if "error" in details:
            LOGGER.error(f"Error in details response: {details}")
        ...
        LOGGER.error(f"Error in details API: {e}")
        import traceback
...
@metadata_api_bp.route('/exists/<provider>/<item_id>', methods=['GET'])
def api_check_exists(provider, item_id):
    """Check if an item already exists in the library.
    
    Args:
        provider: The metadata provider (GoogleBooks, AniList, etc.)
        item_id: The item ID from the provider
        
    Returns:
        Response: Whether the item exists and its series_id if it does
    """
    try:
        from backend.internals.db import execute_query
        
        # Check if the item exists in the series table
        result = execute_query(
            "SELECT id, title FROM series WHERE metadata_source = ? AND metadata_id = ?",
            (provider, item_id)
        )
