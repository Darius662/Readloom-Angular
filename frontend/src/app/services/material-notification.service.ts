...
export class MaterialNotificationService {
  private notifications$ = new BehaviorSubject<MaterialNotification[]>([]);
   ...
   */
  getNotifications(): Observable<MaterialNotification[]> {
  ...
  private addNotification(message: string, type: 'success' | 'error' | 'warning' | 'info', duration: number): void {
    const id = `material-notification-${this.notificationIdCounter++}`;
    const notification: MaterialNotification = { id, message, type, duration };
